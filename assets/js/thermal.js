...
(function () {
  ...
  function cashierCandidates() {
    return uniqNames([PRINTER_CASHIER()].concat(CFG().printerCashierAliases || [
      'RONGTA 80mm 2 على SERVER', 'RONGTA 80mm 2 on SERVER', 'RONGTA80mm2'
    ]));
  }
  ...
  /* ── تصنيف اسم الطابعة: مشاركة شبكة أم محلي ──
     «على SERVER» / on PC / \\HOST\name = مسار شبكة (الجهاز الثاني)
     الاسم القصير بدون لاحقة = تعريف محلي (USB عند التوصيل أو نسخة محلية) */
  function isNetworkPrinterName(name) {
    const s = String(name || '');
    if (/^\s*\\\\/.test(s)) return true;                 /* \\SERVER\Printer */
    if (/\sعلى\s+\S+/i.test(s)) return true;              /* … على SERVER / على PC-POC */
    if (/\son\s+\S+/i.test(s)) return true;               /* … on SERVER */
    if (/\/\//.test(s) && /printer/i.test(s)) return true;
    return false;
  }
  function stripShareSuffix(s) {
    return String(s || '')
      .replace(/\s*على\s+\S+\s*$/i, '')
  ...
  function roleOfName(name) {
    ...
    if (/series/i.test(raw) || /80mm1/.test(n) || /80mmseries/.test(n)) return 'kitchen';
    /* كاشير: 80mm2 أو rongta بدون series */
    if (/80mm2/.test(n) || (/rongta/.test(n) && !/series/i.test(raw))) return 'cashier';
    return null;
  }
  ...
  function candidatesWantRole(candidates) {
    ...
    (candidates || []).forEach(function (c) {
      const raw = String(c || '');
      const n = normPrinter(raw);
      if (/series/i.test(raw) || /80mm1/.test(n) || /80mmseries/.test(n)) kitchen = true;
      else if (/80mm2/.test(n)) cashier = true;
      else if (/rongta/.test(n) && !/series/i.test(raw) && !/80mm1/.test(n)) cashier = true;
    });
    if (kitchen && !cashier) return 'kitchen';
    if (cashier && !kitchen) return 'cashier';
    return null; /* مختلط أو غير معروف — لا نستخدم دورًا عاماً */
  }
  ...
  function nameMatchesCandidates(name, candidates) {
    ...
    /* لا تسمح بتطابق كاشير مع مرشّحي مطبخ والعكس حتى بالاحتواء */
    ...
    for (let i = 0; i < candidates.length; i++) {
      ...
      if (base === stripShareSuffix(c).toLowerCase()) return true;
      if (nrm && nrm === normPrinter(c)) return true;
      const cn = normPrinter(c);
      /* احتواء حذر: لا تربط 80mm2 بـ 80mmseries */
      if (cn.length >= 8 && nrm.length >= 8 && (nrm.indexOf(cn) >= 0 || cn.indexOf(nrm) >= 0)) return true;
    }
    /* دور عام فقط إن اتفق الدور مع مجموعة المرشّحين */
  ...
  function isUsbLive(detail) {
    ...
    if (detail.trays && /usb/i.test(JSON.stringify(detail.trays))) return true;
    return false;
  }
  function isDetailNetwork(detail) {
    if (!detail) return false;
    const blob = JSON.stringify(detail).toLowerCase();
    if (/\bnetwork\b|\bwsd\b|\btcp\b|\bip\b|\bshared\b|\bremote\b/.test(blob)) return true;
    if (detail.connection && /network|tcp|wsd|smb/i.test(String(detail.connection))) return true;
    return false;
  }

  /*
    اختيار اسم الدور تلقائياً — لا يرتبط بجهاز/IP ثابت (آمن عند تبديل الأجهزة):

    1) اجمع كل الطابعات المطابقة للدور من قائمة QZ.
    2) صنّف: شبكة (على XXX / \\…) × محلي (الاسم القصير).
    3) إن وُجد USB حي (من details) → فضّله (الجهاز الموصول بالطابعة).
    4) إن وُجدت مشاركة شبكة + محلي معاً بلا USB حي
         → فضّل الشبكة  ← يحل: مستندات تذهب للمحلي والمشتركة فاضية.
    5) إن وُجد نوع واحد فقط → خذه (الرئيسي غالباً محلي فقط).
  */
  function pickBestForRole(available, candidates, detailsMap) {
    ...
    (available || []).forEach(function (nm) {
      ...
      const netName = isNetworkPrinterName(nm) || isDetailNetwork(d);
    ...
    /* USB حي أولاً — الجهاز الموصول فعلياً */
    ...
    /* الاثنان معاً وبلا USB → الشبكة (وضع الجهاز الثاني) */
    if (nets.length && locs.length) return nets[0].name;
    /* شبكة فقط */
    if (nets.length) return nets[0].name;
    /* محلي فقط (وضع الرئيسي عند التوصيل) */
  ...
  async function resolvePrinters(force) {
    if (!force && resolved.cashier && resolved.host === linkHost && (Date.now() - resolved.at) < 60000) {
      return resolved;
    }
    ...
    console.info('[ThermalPrint] resolve on', linkHost,
      '→ cashier:', resolved.cashier, 'kitchen:', resolved.kitchen,
      '| available:', available);
    return resolved;
  }
