...
(function () {
  ...
  /* الاسم الأساسي + أسماء المشاركة (بدون فراغات) — يختار النظام المتاح تلقائياً */
  ...
  function matchName(available, candidates) {
    ...
    const stripServer = function (s) {
      return String(s || '')
        .replace(/\s*على\s*SERVER\s*/gi, ' ')
        .replace(/\s*on\s+SERVER\s*/gi, ' ')
        .replace(/\s*\\\\[^\\\s]+\\/g, ' ') /* \\PC\share style prefix leftovers */
        .replace(/\s+/g, ' ')
        .trim();
    };
    const norm = function (s) {
      return stripServer(s).toLowerCase().replace(/[\s_\-]+/g, '');
    };
    const av = (available || []).map(function (n) {
      return { raw: n, key: String(n).trim().toLowerCase(), base: stripServer(n).toLowerCase(), nrm: norm(n) };
    });
    /* 1) تطابق تام للاسم كما هو */
    for (let i = 0; i < candidates.length; i++) {
      const c = String(candidates[i]).trim().toLowerCase();
      for (let j = 0; j < av.length; j++) if (av[j].key === c) return av[j].raw;
    }
    /* 2) تطابق بعد حذف «على SERVER» / on SERVER */
    for (let i = 0; i < candidates.length; i++) {
      const c = stripServer(candidates[i]).toLowerCase();
      for (let j = 0; j < av.length; j++) if (av[j].base === c) return av[j].raw;
    }
    /* 3) تطابق بلا فراغات */
    for (let i = 0; i < candidates.length; i++) {
      const c = norm(candidates[i]);
      for (let j = 0; j < av.length; j++) if (av[j].nrm === c) return av[j].raw;
    }
    /* 4) احتواء بعد التطبيع */
    for (let i = 0; i < candidates.length; i++) {
      const c = norm(candidates[i]);
      if (c.length < 4) continue;
      for (let j = 0; j < av.length; j++) {
        const a = av[j].nrm;
        if (a.indexOf(c) >= 0 || c.indexOf(a) >= 0) return av[j].raw;
      }
    }
    /* 5) تمييز كاشير/مطبخ RONGTA عند غياب التطابق الحرفي
       كاشير: فيه 80mm2 أو ينتهي بـ 2 وليس Series
       مطبخ: فيه Series */
    const isKitchenRole = candidates.some(function (c) {
      return /series/i.test(String(c)) || /80mm1/i.test(norm(c));
    });
    const isCashierRole = candidates.some(function (c) {
      const n = norm(c);
      return /80mm2/.test(n) || (!/series/i.test(String(c)) && /rongta/.test(n));
    });
    if (isKitchenRole) {
      for (let j = 0; j < av.length; j++) {
        if (/rongta/i.test(av[j].raw) && /series/i.test(av[j].raw)) return av[j].raw;
      }
    }
    if (isCashierRole) {
      for (let j = 0; j < av.length; j++) {
        if (/rongta/i.test(av[j].raw) && !/series/i.test(av[j].raw) && /80\s*mm\s*2/i.test(av[j].raw)) return av[j].raw;
      }
      for (let j = 0; j < av.length; j++) {
        if (/rongta/i.test(av[j].raw) && !/series/i.test(av[j].raw)) return av[j].raw;
      }
  ...
  async function resolvePrinters(force) {
    ...
    const cash = matchName(available, cashierCandidates());
    const kit = matchName(available, kitchenCandidates());
    resolved = {
      cashier: cash || null,
      kitchen: kit || null,
      at: Date.now(),
    ...
    console.info('[ThermalPrint] resolve on', linkHost, '→ cashier:', resolved.cashier, 'kitchen:', resolved.kitchen, 'from', available.length, 'printers');
    return resolved;
  }
