...
(function () {
  ...
  function cashierCandidates() {
    return uniqNames([PRINTER_CASHIER()].concat(CFG().printerCashierAliases || ['RONGTA80mm2']));
  }
  function kitchenCandidates() {
    return uniqNames([PRINTER_KITCHEN()].concat(CFG().printerKitchenAliases || ['RONGTA80mm1']));
  }
  ...
  function matchName(available, candidates) {
    ...
    const av = (available || []).map(function (n) { return { raw: n, key: String(n).trim().toLowerCase() }; });
    /* 1) تطابق تام */
    for (let i = 0; i < candidates.length; i++) {
      const c = String(candidates[i]).trim().toLowerCase();
      for (let j = 0; j < av.length; j++) if (av[j].key === c) return av[j].raw;
    }
    /* 2) تطابق بلا فراغات/شرطات */
    const norm = function (s) { return String(s).toLowerCase().replace(/[\s_\-]+/g, ''); };
    for (let i = 0; i < candidates.length; i++) {
      const c = norm(candidates[i]);
      for (let j = 0; j < av.length; j++) if (norm(av[j].raw) === c) return av[j].raw;
    }
    /* 3) احتواء */
    for (let i = 0; i < candidates.length; i++) {
      const c = norm(candidates[i]);
